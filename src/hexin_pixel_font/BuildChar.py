import logging
import hexin_pixel_font.LibHexin as LibHexin
import rich
import rich.logging
...
def main():
    ...
    componentIndex = LibHexin.getComponents(
        list(filter(lambda a: a.endswith(".png"), os.listdir("Data/Components")))
    )
    ...
    for char in result:
        ...
        def _generateImage(charData: str):
            ...
            font = ImageFont.truetype("Data/Fonts/Fallback Components.ttf", 8)
            ...
            if full:
                componentImage = Image.open(f"Data/Components/{char}.png")
